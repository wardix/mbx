...
export class ProspectService {
  ...
  async getToken() {
    ...
    form.append(
      'client_secret',
      this.configService.get('PROSPECT_CLIENT_SECRET'),
    );
    ...
    const response = await axios.post(
      this.configService.get('PROSPECT_TOKEN_API_URL'),
      form,
      options,
    );
  ...
  async getOwner(token: string) {
    ...
    const response = await axios.get(
      this.configService.get('PROSPECT_PROFILE_API_URL'),
      { headers },
    );
  ...
  async createLead(name: string, phone: string) {
    ...
    const safeName =
      name.length === 0
        ? this.configService.get('PROSPECT_LEAD_DEFAULT_NAME')
        : name;
    const data = {
      group_id: groups,
    ...
    axios
      .post(this.configService.get('PROSPECT_LEAD_API_URL'), data, { headers })
      .catch((error) => {
